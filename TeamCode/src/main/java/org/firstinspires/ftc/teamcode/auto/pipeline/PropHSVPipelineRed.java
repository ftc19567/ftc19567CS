...
public class PropHSVPipelineRed extends OpenCvPipeline {
    ...
    Scalar lowHSVRed = new Scalar (0.0, 92.1, 63.8);
    // same here. Values can be tuned using eocv sim in valueFinder
    Scalar highHSVRed = new Scalar (225, 255, 255);
    ...
    public Mat processFrame(Mat input) {
        ...
        if (rightPercent > middlePercent && rightPercent > 10) {
            location = Location.RIGHT;
            telemetry.addData("Side: ", "Right");
        } else if (middlePercent > rightPercent && middlePercent > 10) {
